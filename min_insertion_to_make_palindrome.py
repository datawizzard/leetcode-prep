...
# minimum number of insertion to make palindrome == minimum number
# of deletion to make palindrome.
def lps(X,Y,n,m):
	for i in range(n+1):
		for j in range(m+1):
			if i==0 or j==0:
	...
	# 		print(dp[i][j],end=" ")
	#	print()
	# number of deletion = Lengrh of string - Longest Common Subsequence
	return len(X)-dp[i][j]
X=input()
Y=X[::-1]
