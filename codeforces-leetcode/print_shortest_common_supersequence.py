...
def lcs(X,Y,n,m):
	...
	# 		print(dp[i][j],end=" ")
	#	print()
	# just like we print lcs we have to do the same just here change is when
    # both the character are not same then also we have to add that character to 
    # the string.
	s=""
	while i>0 and j>0:
		if X[i-1]==Y[j-1]:
			...
			s=s+X[i-1]
			i-=1
	# Also when either of them i!=0 or j!=0 we have to add that
	# extra character to the string s. 
	while i>0:
		s=s+X[i-1]
		i-=1
	while j>0:
